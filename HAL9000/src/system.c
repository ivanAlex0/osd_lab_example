...
typedef struct _HelloIpiContext {
    QWORD ticks;
}HelloIpiContext, *PHelloIpiContext;
...
#pragma warning(suppress: 28039)
    ...
    int x = 0;
    PPCPU firstCPU = CONTAINING_RECORD(pCpuListHead->Flink, PCPU, ListEntry);
    BYTE group = __inbyte(firstCPU->LogicalApicId);
    ...
    {
       /* if (x % 2 == 0) {
            PPCPU currentCPU = CONTAINING_RECORD(pCurEntry, PCPU, ListEntry);
            LOG("CPU 0x%X\n", currentCPU->LogicalApicId);
