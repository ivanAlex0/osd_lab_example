...
#define DEAD_BEEF                   0x5
#define DEAD_C0DE                   0x6
...
QWORD DeadBeef = DEAD_BEEF;
QWORD DeadC0de = DEAD_C0DE;
...
)
{
    ...
    QWORD shadowSpaceSize = 0;
    QWORD stackSize = 0;

    PVOID UMAddressMappedToKernel;
    PVOID* argumentsAddresses = (PVOID*)ExAllocatePoolWithTag(PoolAllocateZeroMemory, Process->NumberOfArguments * sizeof(PVOID), HEAP_TEST_TAG, 0);
    char* argv[256];

    const char* token;
    const char* delimiter = " ";
    char* fullCommandLine = Process->FullCommandLine;
    token = strtok_s(fullCommandLine, delimiter, &fullCommandLine);

    DWORD currentArgumentIndex = 0;
    while (token != NULL) {

        // [Alex - A2] Assign the current parameter in the argv array
        argv[currentArgumentIndex] = (char*)token;

        // [Alex - A2] Add argument size + 1 (for the appended \0 character)
        stackSize += (QWORD)strlen(token) + 1;

        token = strtok_s(NULL, delimiter, &fullCommandLine);
        currentArgumentIndex++;
    }

    // shadow space stack alignment
    if (Process->NumberOfArguments % 2 == 0) {
        shadowSpaceSize = ((QWORD)PtrDiff(InitialStack, stackSize) % 16);
        stackSize += shadowSpaceSize;
    }
    else {
        QWORD currentStackLastDigit = ((QWORD)PtrDiff(InitialStack, stackSize) % 16);
        shadowSpaceSize = currentStackLastDigit < 8 ? currentStackLastDigit + 8 : currentStackLastDigit - 8;
        stackSize += shadowSpaceSize;
    }

    // [Alex - A2] Add argc size (number of arguments)
    stackSize += (QWORD)Process->NumberOfArguments * sizeof(PVOID);

    // [Alex - A2] Add the shadow space size(2)
    // Size += SHADOW_STACK_SIZE;
    stackSize += (QWORD)2 * sizeof(PVOID);

    // [Alex - A2] Add size for the three special items: argc, argv and return address
    stackSize += (QWORD)3 * sizeof(PVOID);


    // [Alex - A2] Allocate the computed size by making a diff between the initialStack and the stackSize
    *ResultingStack = (PVOID)PtrDiff(InitialStack, stackSize);


    // ----------------------------------------------------------
    // [Alex - A2] Map the physical memory which backs UserAddress to Kernel
    MmuGetSystemVirtualAddressForUserBuffer(*ResultingStack, stackSize, PAGE_RIGHTS_READWRITE, Process, &UMAddressMappedToKernel);


    // [Alex - A2] Keep the address of the kernelAddress to free it later
    PVOID InitialKernelAddressPointer = UMAddressMappedToKernel;

    // [Alex - A2] Go to the initial address of the stack by adding the stackSize computed
    UMAddressMappedToKernel = (PVOID)PtrOffset(UMAddressMappedToKernel, stackSize);

    PVOID InitialStackPointer = UMAddressMappedToKernel;

    // [Alex - A2] Start pushing the arguments on the stack using memcpy
    for (QWORD j = Process->NumberOfArguments; j >= 1; j--) {

        QWORD currentIndex = j - 1;
        QWORD argumentSize = (QWORD)strlen(argv[currentIndex]) + 1;

        UMAddressMappedToKernel = (PVOID)PtrDiff(UMAddressMappedToKernel, argumentSize);

        argumentsAddresses[currentIndex] = (PVOID)PtrDiff(InitialStack, ((QWORD)PtrDiff(InitialStackPointer, UMAddressMappedToKernel)));
        memcpy(UMAddressMappedToKernel, argv[currentIndex], argumentSize);
    }

    // // [Alex - A2] Push on the stack the value of the shadowSpaceSize - the shadow space
    UMAddressMappedToKernel = (PVOID)PtrDiff(UMAddressMappedToKernel, shadowSpaceSize);

    UMAddressMappedToKernel = (PVOID)PtrDiff(UMAddressMappedToKernel, sizeof(PVOID));

    // [Alex - A2] Now push the addresses of the arguments on stack
    for (QWORD j = Process->NumberOfArguments; j >= 1; j--) {
        ...
        status = MmuIsBufferValid(&argumentsAddresses[currentIndex], sizeof(&argumentsAddresses[currentIndex]), PAGE_RIGHTS_READ, GetCurrentProcess());

        memcpy(UMAddressMappedToKernel, &argumentsAddresses[currentIndex], sizeof(argumentsAddresses[currentIndex]));
        UMAddressMappedToKernel = (PVOID)PtrDiff(UMAddressMappedToKernel, sizeof(PVOID));
    }

    // [Alex - A2] Push the DEAD_BEEF (shadow space) on the stack --> 2 times
    memcpy(UMAddressMappedToKernel, &DeadBeef, sizeof(PVOID));
    UMAddressMappedToKernel = (PVOID)PtrDiff(UMAddressMappedToKernel, sizeof(PVOID));
    ...
    // [Alex - A2] Push the address of the first argument on the stack
    PVOID AddressOfArgv0 = (PVOID)PtrDiff(InitialStack, ((QWORD)PtrDiff(InitialStackPointer, UMAddressMappedToKernel)));
    memcpy(UMAddressMappedToKernel, &AddressOfArgv0, sizeof(PVOID));
    UMAddressMappedToKernel = (PVOID)PtrDiff(UMAddressMappedToKernel, sizeof(PVOID));


    // [Alex - A2] Push the number of arguments on the stack
    memcpy(UMAddressMappedToKernel, &Process->NumberOfArguments, sizeof(PVOID));
    UMAddressMappedToKernel = (PVOID)PtrDiff(UMAddressMappedToKernel, sizeof(PVOID));

    // [Alex - A2] Push the fake return address DEAD_C0DE
    memcpy(UMAddressMappedToKernel, &DeadC0de, sizeof(PVOID));
    UMAddressMappedToKernel = (PVOID)PtrDiff(UMAddressMappedToKernel, sizeof(PVOID));


    // [Alex - A2] Free the previously mapped virtual address from kernel
    MmuFreeSystemVirtualAddressForUserBuffer(InitialKernelAddressPointer);
